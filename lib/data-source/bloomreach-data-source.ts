...
//
// The Bloomreach adapter — implements the DataSource interface over a connected
// MCP transport. Carries the OAuth/PKCE/DCR-driven Bloomreach loomi connect
// session (handled in lib/mcp/auth.ts + lib/mcp/connect.ts), the ~1 req/s
// proactive spacing, the rate-limit retry ladder (parses the server's stated
// window, falls back to backoff), the AbortSignal composition for
// cancellation (PR #5), and the 60s response cache that absorbs repeats.
//
// History: this was originally `McpClient` in `lib/mcp/client.ts` — renamed +
// moved to `lib/data-source/` in PR A of Phase 2 (the DataSource seam). The
// internals are unchanged on purpose: the seam wasn't retrofitted, the class
// was already shaped to be the Bloomreach adapter. Lifting `DataSource` over
// it only changed the TYPE that callers consume.
import type { McpTransport } from '../mcp/transport';
import type { DataSource } from './types';

/** Per-call options accepted by BloomreachDataSource. Adds `cacheTtlMs` and
 *  `skipCache` on top of the abstract DataSource surface — these are
 *  Bloomreach-specific cache controls used by /api/mcp/call + /api/mcp/capture
 *  and a couple of dev tooling paths. The agents only ever pass `signal`. */
...
/** Result envelope returned by BloomreachDataSource.callTool. The generic `T`
 *  lets call sites narrow the `result` field at the call site (e.g.
 *  `client.callTool<MyShape>(...)`); the abstract DataSource interface drops
 *  the generic and types `result` as `unknown`. */
...
/** BloomreachDataSource — the Bloomreach adapter for the DataSource interface.
 *  Wraps a connected MCP SDK transport (with OAuth/PKCE/DCR session) and adds:
 *    - 60s response cache (per name+args)
 *    - ~1 req/s proactive spacing (Bloomreach rate-limits per user globally)
 *    - rate-limit retry ladder honoring the server's stated penalty window
 *    - AbortSignal composition (route-level cancel OR per-call 30s timeout)
 *    - typed McpToolError for transport/tool failures
 *  Aliased as `McpClient` from `lib/mcp/client.ts` so existing imports compile
 *  unchanged during the seam-extraction PR. */
