...
Updated: 2026-06-19 — Olist removal (PR #8) reverted two-pillar framing to one pillar. 269/28 → 221/24 tests. AI-eval flag flipped back to present (no eval suite). Flag 12 (parallel-run hazard) marked n/a — pipeline gone. Top-3 findings repointed: model-quality gap is the new top finding (replaces production-MCP eval gap, which is no longer applicable); contract test gap unchanged at #2; Anthropic throw-path gap promoted to #3 (replaces parallel-run hazard, now n/a).
