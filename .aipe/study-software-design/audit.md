...
---
Updated: 2026-06-19 — Olist removal landed (PR #8) — `mcp-server-olist/` sibling package, `OlistDataSource` adapter, and `eval/` directory all removed; verdict rewritten to lead with "DataSource seam survived two adapter swaps"; SyntheticDataSource (516 LOC) added as second DataSource adapter; AptKit primitive adapter boundary (`lib/agents/aptkit-adapters.ts`, 206 LOC, three small classes) named as the new highest-leverage APOSD case study with deep walk in new `05-aptkit-primitive-adapter-boundary.md`; the four active agent classes shrunk to 30–50 LOC instantiate-and-forward shims; `*-legacy.ts` parallel files named in red-flags as the new LOW-priority "decide retirement" item; top-3 findings refreshed.
