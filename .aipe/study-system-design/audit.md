...
---
Updated: 2026-06-19 — refreshed verdict to drop the eval-suite framing (removed in PR #8) and replace the "two live backends over subprocess" story with "two live backends: Bloomreach over HTTPS, Synthetic in-process"; added the AptKit primitive boundary as the fifth load-bearing piece. `system-map-and-boundaries` rewritten: down from four real boundaries to three (the Olist subprocess boundary B2′ is gone), AptKit primitive boundary called out as new and load-bearing, `lib/agents/aptkit-adapters.ts` named as the sole file that knows about both Anthropic and AptKit shapes. `request-response-and-data-flow` dropped Flow 4 (eval) and re-stated the three flows as branching on `bi:mode` with `live-synthetic` finishing in seconds vs `live-bloomreach`'s 30–120s. Tier 7′ in `storage-choice-and-durability-boundaries` updated from "committed SQLite seed" to "in-process fixture constants in `synthetic-data-source.ts`" — same "git as storage tier" pattern, different shape. Cross-links into retired files (`09-eval-pipeline.md`, `10-authored-mcp-server.md`) removed from the active body; the RETIRED files are kept as historical artifacts. Test count 269 → 221 (Olist test removal then +5 synthetic adapter tests).
