...
> **Verdict-first.** blooming insights is a **Next.js 16 app that hangs four Claude agents off a swappable MCP backend, with no database**. As of 2026-06 it runs over TWO real backends — Bloomreach Engagement (live HTTPS MCP, OAuth) in prod, and an authored `mcp-server-olist` subprocess (stdio, SQLite-backed) for eval — picked at runtime by `bi:mode = 'demo' | 'live-sql' | 'live-bloomreach'` through a `DataSource` interface in `lib/data-source/types.ts`. The architecture earns its weight from FOUR pieces that show up in every lens below: `runAgentLoop` (one function, four agents — see `06-multi-agent-orchestration.md`), `BloomreachDataSource` (the Bloomreach adapter with TTL cache + spacing + retry — see `04-caching-and-rate-limiting.md`), the route-level NDJSON streams (long agent runs feel responsive — see `05-streaming-ndjson.md`), and a 4-pillar eval suite (detection / diagnosis / recommendation / regression — see `09-eval-pipeline.md`). The strongest pattern is the absence of a database — state lives in in-memory `Map`s + an encrypted cookie + committed demo JSON + a committed 3.6 MB SQLite seed in `mcp-server-olist/data/olist.db`. The load-bearing gap is the same `Map` being global to the instance rather than session-keyed — at ~10 concurrent users on one warm Vercel instance, one briefing's `putInsights.clear()` wipes another's. Highest-priority finding: session-key `lib/state/insights.ts` (~30 LOC, eliminates the only correctness issue at any current scale).

---

## system-map-and-boundaries

Six bands now (was five — added since 2026-06). UI is two pages and one hook (`app/page.tsx` L87–L817, `app/investigate/[id]/page.tsx`, `lib/hooks/useInvestigation.ts` L37–L216). Route layer is three NDJSON-streaming handlers, all `maxDuration = 300` (`/api/briefing`, `/api/agent`, `/api/mcp/*`). Agent layer is one shared loop + four agent classes (`lib/agents/base.ts` L48–L176 + monitoring/diagnostic/recommendation/query). **DataSource band (NEW)** — `lib/data-source/types.ts` defines the three-method interface; `makeDataSource(mode, sessionId)` in `lib/data-source/index.ts` picks the adapter; two real implementations (`BloomreachDataSource` formerly `McpClient`, now `lib/data-source/bloomreach-data-source.ts`; `OlistDataSource` in `lib/data-source/olist-data-source.ts`). `lib/mcp/client.ts` is a 17-line backwards-compat shim. Provider layer: `BloomreachDataSource` wraps `McpTransport` wrapping the MCP HTTP SDK with `BloomreachAuthProvider` (`lib/mcp/auth.ts` L86–L218) underneath; `OlistDataSource` spawns `mcp-server-olist` (sibling package, ~1800 LOC, SQLite + stdio) as a subprocess. External is Bloomreach MCP (data, ~1 req/s/user GLOBAL), the Olist subprocess (data, no rate limit), and Anthropic (reasoning, `claude-sonnet-4-6`). **Sibling: `eval/`** (~75 files) — a parallel-universe agent runner that drives the same agent layer through `OlistDataSource` for 4-pillar measurement.

**Four real boundaries, one cosmetic.**
...
- **B2 BloomreachDataSource ↔ Bloomreach** — trust flips OURS → UPSTREAM + rate flips to ~1 req/s/user. The whole Bloomreach path is shaped by this latency floor. (The Olist path has no equivalent — SQLite reads are local and rate-limit-free.)
- **B2′ OlistDataSource ↔ subprocess (NEW)** — trust flips IN-PROCESS → CHILD-PROCESS. Stdio JSON-RPC frames; subprocess is read-only on the DB. No auth, no rate limit, ~10–50 ms per call. The subprocess lifecycle is one-per-instance, lazy-connect, killed on `dispose()`.
...
→ see `02-oauth-boundary.md` for the B1 mechanism · `04-caching-and-rate-limiting.md` for the B2 defense · `study-security/` for the B3 enforcement detail · `03-provider-abstraction.md` for the new `DataSource` upper seam + the still-valid `McpTransport` lower seam · `10-authored-mcp-server.md` for the B2′ subprocess mechanism · `09-eval-pipeline.md` for the eval sibling.

---

## request-response-and-data-flow

Three live flows + one replay shortcut + one out-of-band eval pipeline, all sharing the same agent code. The three production flows ride NDJSON `AgentEvent` (`lib/mcp/events.ts` L4–L17); the eval pipeline is stdio capture instead.

- **Flow 1 — briefing (`/api/briefing`).** Schema bootstrap → coverage gate → monitoring scan → emit `workspace`, `coverage_item ×10`, `tool_call_*`, `insight×N`, `done`. Sequential, 30–60s cold start (Bloomreach); ~5–15s under `live-sql`.
...
- **Flow 4 — eval (out-of-band, NEW since 2026-06).** `npm run eval:<pillar>` → spawn `OlistDataSource` per K iteration → run the same agent code as flow 1/2/3 → judge (deterministic for detection, LLM for diagnosis/recommend/regression) → write `eval/results/<date>[/<tag>]/*.json + summary.md`. Sequential K=10, ~5–10 min wall-clock per pillar. **All three production flows now branch on `bi:mode` at the route level** — `live-bloomreach` runs the original Bloomreach path; `live-sql` calls `makeDataSource('live-sql', sid)` which spawns the Olist subprocess instead of opening an OAuth-gated MCP transport.
...
→ see `01-request-flow.md` for the briefing hop-by-hop (now with three modes) · `05-streaming-ndjson.md` for the wire format mechanics · `07-client-stream-handoff.md` for the client-side reader + step-to-step handoff · `06-multi-agent-orchestration.md` for the CODE → MODEL flip inside the loop · `09-eval-pipeline.md` for flow 4 (the parallel-universe request flow with judge + paper trail).
...
- **Tier 7′ — committed SQLite (NEW).** `mcp-server-olist/data/olist.db` (3.6 MB) — Brazilian e-commerce seed data + `seeded_anomalies` ground truth. Committed because the eval pipeline depends on stable seed across machines (`09-eval-pipeline.md`). Same "git as storage tier" pattern, different shape.
...
3. **Circuit breaker on `BloomreachDataSource`** — `lib/data-source/bloomreach-data-source.ts` (new state on the class) — track consecutive 5xx per tool; after 5 in a row, open the circuit for 30s and fail fast; close on first success. ~50 LOC, no external dependency; prevents a Bloomreach outage from eating the 300s budget on every concurrent request. (Olist adapter doesn't need this — SQLite never 5xxes.)

---
Updated: 2026-06-16 — refreshed verdict to name the DataSource seam + the two live modes (`live-sql`/`live-bloomreach`) + the 4-pillar eval suite as load-bearing pieces alongside `runAgentLoop` and `BloomreachDataSource`; updated `system-map-and-boundaries` to six bands (added DataSource band, added the `eval/` sibling, added the B2′ subprocess boundary for Olist); added Flow 4 (eval out-of-band) to `request-response-and-data-flow`; added Tier 7′ (committed SQLite at `mcp-server-olist/data/olist.db`) to `storage-choice-and-durability-boundaries`; renamed `McpClient` references to `BloomreachDataSource` where the relocation matters. The "provider abstraction has no real seam in this codebase" framing (if it was in the audit's earlier scope) is now RESOLVED — the seam ships two real adapters and a runtime mode switch. Test count grew 144 → 269.
