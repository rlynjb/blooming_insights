...
Updated: 2026-06-19 — rewrote the verdict paragraph around the AptKit primitive boundary (`@aptkit/core@0.3.0` + three Blooming bridge classes in `lib/agents/aptkit-adapters.ts`) and the in-process `SyntheticDataSource` replacing the retired Olist subprocess. The system diagram now shows the agent band as five thin classes built on AptKit + adapter bridges (no more `runAgentLoop` as the visible primitive), the DataSource band as two adapters side-by-side (Bloomreach over HTTPS+OAuth, Synthetic in-process), and the external providers band trimmed to Bloomreach + Anthropic (no `mcp-server-olist`, no eval/ sibling). Move 2 entries for the agent + provider bands rewritten to reflect AptKit + Synthetic. Move 3 reframed as "external constraint + internal discipline" — the discipline is the "generic primitive + domain adapter" pattern applied at TWO seams (`DataSource` and `@aptkit/core`). Implementation table reordered: 8 active + 2 new (11, 12) + 2 RETIRED (09, 10). Elaborate "What this map doesn't show" rewritten to teach the two-seam adapter lesson applied twice. The "no database" framing simplified — the eval-ground-truth SQLite seed is gone too, so the only durable tiers are cookies + committed demo JSON + Bloomreach (upstream).
