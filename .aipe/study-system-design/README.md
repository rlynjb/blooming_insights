...
Updated: 2026-06-19 — verdict rewritten: dropped the eval-suite framing (removed in PR #8), replaced the Olist subprocess story with the in-process Synthetic adapter, added the AptKit primitive boundary as the fifth load-bearing piece. Reading order updated: `01-request-flow.md` one-liner mentions three modes (was `live-sql`); `03-provider-abstraction.md` one-liner says "three implementations" (was "two"); `06-multi-agent-orchestration.md` one-liner mentions the AptKit refactor. Added [11-aptkit-primitive-adapters.md](./11-aptkit-primitive-adapters.md) and [12-synthetic-data-source.md](./12-synthetic-data-source.md) as new pattern files. `09-eval-pipeline.md` and `10-authored-mcp-server.md` kept with RETIRED banners as historical artifacts and called out as such in the reading list. Pass 2 active pattern file count: 10 (8 carried + 2 new); 2 RETIRED.
