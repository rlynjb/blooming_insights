...
**Where this lives in the repo.** `McpClient.liveCall` in `lib/mcp/client.ts` L148–L163 (the spacing gate at L149–L151; transport call at L154; `lastCallAt` update at L155). Single-instance field `lastCallAt` (L81) tracks the last send; every live call waits the remainder of `minIntervalMs` before hitting the transport. The spacing configuration: `connectMcp`'s `McpClient` constructor call in `lib/mcp/connect.ts` L89–L96 — the options object includes `minIntervalMs: 1100`. The sequential agent topology that makes serial spacing sufficient: `app/api/agent/route.ts` `GET` stream `start()` body L224–L249 (diagnostic → recommendation; the `if`-ladder that picks the next agent).
