...
Updated: 2026-06-19 — Reflected AptKit v0.3.0 migration: active spine is now `@aptkit/core`'s agent classes; Blooming's `runAgentLoop` is preserved at `base-legacy.ts` for the legacy path. Documented the 3 adapter classes in `aptkit-adapters.ts` as the Blooming-owned bridge. Removed mcp-server-olist + eval/ references (both deleted in PR #8). Added `SyntheticDataSource` as the second adapter behind the surviving DataSource seam. Test count: 221.
