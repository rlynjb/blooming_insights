...
5. **Unit semantics live outside the schema — the BRL cents-vs-Reais bug.** A shape contract only proves the field is a string or a number; it does not prove the *units* are what downstream code expects. The Olist adapter returns BRL monetary values as integer cents (e.g. `12450000` means R$ 124 500,00) — `monitoring.md` tells the agent "Divide by 100 when narrating in `impact`", but nothing in `isAnomalyArray` enforces it. A model that narrates `current_value_brl_cents: 4200000000` as "revenue of R$ 4.2 billion" emits a perfectly shape-valid `Anomaly` whose `impact` string is off by 100x. The schema is satisfied; the human reading the briefing is misled. This is the unit-narration gap structured-output validators do not cover, and one of the prompt-engineering targets surfaced by the eval suite (→ 05-eval-driven-iteration.md). The fix is prose ("BRL values are stored as cents — divide by 100 before stating") layered with an output-side parser that scales known-cents fields by 0.01 before they reach the UI — the validator route, not the model route, where unit invariants actually hold.
