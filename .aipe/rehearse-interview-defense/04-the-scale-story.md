...
The second thing that breaks is mine, and I own it cleanly — and the sharpest part of it is a real correctness bug, not just a cache-coherence wart: **my state is a global per-instance `Map`, and `putInsights.clear()` wipes other concurrent users on the same warm instance.** That's `lib/state/insights.ts` line 4 (one `Map<id, Insight>` global to the instance) and line 36 (`insights.clear()` at the top of every briefing write). For one user it's correct — each run is the current feed, not an addition — but for two users on one warm Vercel instance, user A's briefing clear-then-set wipes user B's data mid-session, showing up as feed flicker or an investigation 404. The fix is small (~30 LOC: change `Map<id, Insight>` to `Map<sessionId, Map<id, Insight>>` and thread `sessionId` — already available via `getOrCreateSessionId` — through the three call sites). Around it, the rest of the per-instance story still applies: the 60s response cache lives inside one `McpClient` instance (`lib/mcp/client.ts`, the `cache` Map on line 80), cached investigations live in a module-level `Map` in `lib/state/investigations.ts` (line 11), a cold start re-bootstraps the schema from scratch, and two requests on different instances share nothing. I already worked around the cross-instance piece — an investigation's source insight is handed to the agent route through the URL (`?insight=`), parsed in `resolveAnomaly` (`lib/api/agent` route, lines 37-62), *because* I knew in-memory state would not survive Vercel's per-instance memory between the feed and the investigate page. That is a band-aid, not a store.
...
I separate two things here, because interviewers conflate them and I want to show I do not: **perceived latency** and **wall-clock latency.** I already won the perceived-latency fight by design. The whole system streams its reasoning as NDJSON over a `ReadableStream` — the briefing and agent routes enqueue events line by line (`encodeEvent` in `lib/mcp/events.ts`, consumed by a fetch + reader loop in `app/page.tsx`, not EventSource). The user sees "reading the workspace schema…", then each category resolving, then the live EQL queries, *as they happen.* Time-to-first-token is a few hundred milliseconds. Nobody stares at a spinner for two minutes; they watch an analyst think. That is the single biggest latency lever and it is already pulled.

The wall-clock ceiling is the honest other half. A full run is **~1.1s inter-call spacing × number of tool calls**, plus model time per turn, capped by the **300s function limit.** That is why the tool-call budgets are low — they are not just about cost, they bound latency. The 60s TTL cache (`lib/mcp/client.ts`, `cacheTtlMs ?? 60_000` on line 103) absorbs repeats: if two analysts check the same category inside a minute, the second pays zero. Critically, the cache does the right thing under load — it is **never written on an error** (`lib/mcp/client.ts` lines 137-138: an `isError` result returns without caching), so a transient 429 during a burst can never poison the cache and serve a stale error to the next caller. The piece of "model time" I would flag honestly if pushed on cost: the `synthesize()` fallback in `lib/agents/diagnostic.ts` (L87–126) and `lib/agents/recommendation.ts` (L82–132) is an output-heavy structured-JSON call that fires whenever the loop fails to emit valid JSON, and I have no `res.usage` logging on any Anthropic call site — so I can name it as the suspected cost concentration but I cannot quote a number. ~5 lines of `console.log` would unblock that.
...
If I were taking this past a demo and toward real multi-user load, there are two changes — and the cheap one comes first. The cheap one is the ~30 LOC session-key on `lib/state/insights.ts`: turn the global `Map<id, Insight>` into `Map<sessionId, Map<id, Insight>>` and thread the session id (already present via `getOrCreateSessionId`) through `putInsights` / `getInsight` / `listInsights`. That eliminates the only real correctness bug at any current scale — the `putInsights.clear()` wipe of a concurrent user — and it needs no new infra. The bigger one is the same shape I'd lift later: the 60s cache, the cached investigations, and the OAuth PKCE/token state all survive only inside one warm instance, and I'm already routing an insight through the URL because I knew that. I would put a shared store (Redis to start) behind the cache and the investigation store, and move the auth state there too, which would also let me re-enable the CSRF `state` validation that is written and tested but currently unwired (`consumeState` in `lib/mcp/auth.ts`, lines 230-235) because the SDK calls `state()` multiple times per flow and a shared store could track issued states properly. What I would NOT change preemptively is anything to fight the upstream rate limit — that is Bloomreach's number, not mine, and the right response is more independent user budgets and harder caching, not a faster loop.

---

## Summary — Chapter 4

**Core claim:** At 10x the first ceiling is upstream (Bloomreach's ~1 req/s-per-user limit + token revocation), my own first bottleneck is per-instance in-memory state, and I have not built horizontal-scale infra — I know exactly where it breaks and what I'd measure before adding anything.

**Questions covered:**
- *What breaks first at 10x users?* — Bloomreach's per-user rate limit + token revocation (not my code); then `lib/state/insights.ts` line 4/36 — the global `Map` plus `putInsights.clear()` wipes a concurrent user on the same warm instance (~30 LOC session-key fix); then the broader per-instance cache/investigation state, addressed by Redis when I go multi-instance.
...
- *What about 10x latency-sensitive requests?* — Perceived latency is already solved by NDJSON streaming; wall-clock ceiling is ~1.1s × call-count under 300s; the 60s cache absorbs repeats. Measure p50/p95 and call-count.
...
- "Perceived latency is already solved by NDJSON streaming; the wall-clock ceiling is ~1.1s × call-count under the 300s cap."
...
- "The smallest real correctness bug at any current scale: `putInsights.clear()` on a global Map wipes a concurrent user. ~30 LOC of session-keying fixes it before any infra is needed."

**What you'd change:** Session-key the insights `Map` in `lib/state/insights.ts` first (~30 LOC, fixes the concurrent-user clear-wipe with no infra). Then lift state (cache, investigations, OAuth tokens) out of per-instance memory into a shared store when I go multi-instance. Don't build anything to fight the upstream rate limit — that number isn't mine.
