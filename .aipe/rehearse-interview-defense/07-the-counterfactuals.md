...
Updated: 2026-06-20 — MAJOR reframes: (1) Would-not-change list rewritten — `shared runAgentLoop` REMOVED (it's legacy now at base-legacy.ts); REPLACED with `DataSource seam + adapter pattern` (survived 2 swaps without changing caller surface) and `AptKit primitive boundary` (3 small adapters; library owns the loop, I own the boundary). (2) Decision 1 (no-DB) updated: concurrent-user wipe bug is RESOLVED via session-keying; what remains is cross-instance. (3) "What you'd change" eval-harness reframe: the harness was BUILT in Phase 3, used the flywheel to surface 3 real bugs (BRL cents-vs-Reais, calibration binary, conclusion instability), retired with the Olist substrate. New version goes against the Synthetic substrate — that's the version-two story, not a from-scratch keystone. (4) Afternoon list (insights.ts session-key, res.usage logging, e.stack stripping) all shipped — named as such.
