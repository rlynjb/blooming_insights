...
There's a re-ranking I've come around to that's worth saying out loud, because it's the senior framing of "what would you do differently." The instinct is to lead with the session-key fix — it's the smallest, the cheapest, fixes a real correctness bug. And I'd still ship it first as a *correctness* fix. But the move that unlocks every other refactor in this book is the one I didn't ship: **the eval harness.** Five substrate seams are already cut — the NDJSON trace as a span sequence, the replay store, the type-validated agent outputs, the scripted-Anthropic fake harness, the `McpCaller` interface — and the keystone module (`evals/`, with a 10-case goldset + LLM-as-judge + measured agreement rate) is the only one missing. Without it I can ship the session-key fix and the cost meter and the circuit breaker, but I can't *know* the cost meter didn't change agent behavior, can't *know* the breaker didn't drop accuracy, can't *know* a prompt edit regressed. Every refactor downstream of that gap is rate-limited by the same missing measurement. So if I'm honest about ordering, the real "what would I do differently" answer is: **build the eval harness first** (~5 hours, recipe written in `.aipe/drills/evals-observability-induce-eval-gap-build-min-eval-harness.md`), *then* ship the session-key fix and the cost meter knowing I can prove neither broke anything.

If the question is the more concrete *"what would you do this afternoon?"* — three S-effort fixes, ~110 LOC total: (1) session-key `lib/state/insights.ts` (~30 LOC, eliminates the only correctness bug at current scale), (2) `console.log({ usage: res.usage })` at the four Anthropic call sites (~5 LOC, unblocks cost diagnosis on the suspected `synthesize()` concentration), and (3) strip `e.stack` from the four `app/api/mcp/*` error JSON bodies (~4 LOC, closes the info-disclosure leak across the trust boundary). That's the afternoon list, and I'd say it that way — not as regrets, but as the next three commits.

The alias/normalization layer for coverage deps is the next-cheapest beyond the afternoon — clearest trigger (a second workspace), fix contained to `schemaCapabilities`. The bigger DB and the limiter wait on scale or stable traffic I don't have yet, and the demo-replay waits on the upstream stabilizing — none of those are worth building speculatively. The honest through-line is that every one of these was the right call *for an alpha, single-upstream demo*, and each has a named trigger that flips the decision. That's the posture I'd want them to walk away with: not that I got everything right, but that I know precisely what would have to change for each call to be wrong — and that the eval harness is the keystone that lets me know any of the changes worked.
...
**The "what you'd change" sentence:** Build the eval harness first (~5 hours, recipe written) — it's the keystone that lets me prove every other refactor preserved behavior. *Then* ship the afternoon list (session-key `insights.ts` ~30 LOC, `res.usage` logging ~5 LOC, strip `e.stack` from the 4 `app/api/mcp/*` routes ~4 LOC). Then the coverage alias layer. The DB, the limiter, and the replay decisions wait for scale, stable traffic, or a stable upstream.
...
Updated: 2026-06-03 — Adopted the audit-refactor "eval-first re-ranking" in the "what you'd change" closer: the eval harness is the keystone module that unlocks shipping the session-key fix, cost meter, and breaker with evidence; five substrate seams already cut, only the keystone missing. Added the cleanup-2026-06-02 "afternoon list" (3 S-effort fixes, ~110 LOC: session-key insights.ts, `res.usage` logging, `e.stack` stripping) as concrete grounding for "what would you do this afternoon." One-page summary updated.
