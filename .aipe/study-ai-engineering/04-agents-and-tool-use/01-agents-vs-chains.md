...
> blooming insights uses both shapes deliberately: a deterministic CHAIN sequences the agents at the top (monitoring→diagnostic→recommendation in `route.ts`), and inside each node an AGENT loop lets the model decide which tools to call and how many — chain at the top, bounded agent at each node.
...
blooming insights picks neither extreme. It runs a 2-step chain (`investigate` then `propose`) where each step is an agent that runs a *bounded* loop. One-line summary: **the topology is a chain; the nodes are agents; the boundary between them is the source of both predictability and adaptivity.**

---

## How it works

**Mental model.** Think of the route as a reducer with two hard-coded `dispatch` calls, and each agent as a `useReducer` whose actions are chosen at runtime by a model instead of by user clicks. The outer flow is deterministic — you can read `route.ts` and know diagnostic always runs before recommendation. The inner flow is data-driven — you cannot read `base.ts` and know how many tool calls the model will make, because that depends on what the model sees.

```
TOP LEVEL = CHAIN (code owns the order)            NODE LEVEL = AGENT (model owns the order)
────────────────────────────────────────          ────────────────────────────────────────
route.ts:                                           runAgentLoop (base.ts):
  diagnosis = await diag.investigate(inv)             for turn in maxTurns:
  send(diagnosis)                                       res = model.create(messages + tools)
  recs = await rec.propose(inv, diagnosis)              if res has tool_use:
  for r in recs: send(r)                                  run them, feed back  ← model chose
                                                        else: return            ← model chose
fixed: always investigate → propose                 variable: 0..6 tool calls, model decides
```
...
The route is the only place agents are sequenced, and it does so with plain `await`. There is no graph library, no state machine, no framework — two sequential `await` calls with the diagnosis threaded from the first into the second.

```
app/api/agent/route.ts — start(controller)  (L133–L162)
─────────────────────────────────────────────────────────
 if (q && !insightId)                          ← branch A: free-form query
   ...
   diagnosis = await DiagnosticAgent.investigate(inv)   ← node 1
   send({ type:'diagnosis', diagnosis })
   stepFor('recommendation', 'thought', …)
   recs = await RecommendationAgent.propose(inv, diagnosis)  ← node 2 (consumes node 1)
   for r of recs: send({ type:'recommendation', recommendation:r })
   send({ type:'done' })
   saveInvestigation(insightId, collected)
```

The order is hard-coded. `propose` receives `diagnosis` as an argument (route.ts L158) — the data flows through a function parameter, not a shared blackboard. This is a chain in its purest form: step N+1 reads step N's typed output. Note the morning-briefing's *third* agent, monitoring, runs in a different route (`app/api/briefing/route.ts`) and produces the `Insight` rows that become the `insightId` this route investigates — the full briefing pipeline monitoring→diagnostic→recommendation is a chain spread across two routes.
...
When `runAgentLoop` returns text that does not parse as the required JSON shape, `DiagnosticAgent.investigate` runs a second, dedicated model call: `synthesize()` (`diagnostic.ts` L82–L121). That second call has no tools and no loop — it is a single deterministic step that takes the gathered evidence and asks for JSON only.

```
DiagnosticAgent.investigate  (diagnostic.ts L44–L78)
─────────────────────────────────────────────────────────
 step 1 (AGENT):  runAgentLoop(...)         → { finalText, toolCalls }   L50
 step 2 (CHAIN):  tryParseDiagnosis(finalText)        ← parse the loop output  L74
                  ?? await synthesize(anomaly, toolCalls)  ← fixed fallback call  L75
                  ?? FALLBACK                          ← static default          L76
```

Steps 1→2 are a 2-step micro-chain: a variable-length agent loop followed by a fixed synthesis step. `recommendation.ts` L69–L77 has the identical shape. This is the cleanest demonstration of "chain of agents" at the smallest scale — one agent's public method is itself an agent step chained to a deterministic step.

---

### Current state vs future state

Today the chain is exactly two business steps (`investigate → propose`) plus the monitoring step in the sibling route. Sequential `await` is the right tool: there is no branching, no parallelism, no conditional skip. The moment the chain needs to *branch* — for example, "if `diagnosis.confidence` is low, re-investigate with a different tool subset instead of recommending" — the `if/await/if/await` structure becomes a hand-rolled state machine and should move to a graph runner (LangGraph). That breakpoint is named in Tradeoffs; it has not been crossed.
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  ROUTE LAYER (CHAIN — code owns order)   app/api/agent/route.ts       │
│                                                                       │
│  branch A: q only ───→ classifyIntent ─→ QueryAgent.answer ─→ done    │
│  branch B: insightId                                                  │
│     await DiagnosticAgent.investigate(inv) ───┐  (fixed step 1)       │
│     send(diagnosis)                           │                       │
│     await RecommendationAgent.propose(inv, ───┘  (fixed step 2,       │
│            diagnosis) ──→ send(recs) ──→ done     consumes step 1)    │
└───────────────────────────────┬───────────────────────────────────────┘
...
- **Function / class:** `GET` → the `ReadableStream` `start(controller)` body
- **Line range:** L145–L162 (investigation chain); L135–L143 (single-agent query branch)
- **Role:** Sequences `DiagnosticAgent.investigate` then `RecommendationAgent.propose` with plain `await`; `diagnosis` is passed into `propose` as an argument (L158). No graph, no framework — `if/await/await`.
...
- **Function / class:** `DiagnosticAgent.investigate` (agent step) → `synthesize` (chain step)
- **Line range:** L50 (`runAgentLoop`) → L73–L77 (`tryParseDiagnosis ?? synthesize ?? FALLBACK`); `synthesize` body L82–L121
- **Role:** A bounded agent loop followed by a fixed, tool-less synthesis call. Same shape in `recommendation.ts` L69–L77 / L82–L127.
...
**Pseudocode — chain of bounded agents** (route.ts L152–L159 + diagnostic.ts L73–L77):

```typescript
// CHAIN (route.ts): fixed order, typed handoff
...
return tryParseDiagnosis(finalText)         // micro-chain step 1
...
- **What to build:** In `app/api/agent/route.ts`, branch on `diagnosis.confidence`-equivalent signal: if the diagnosis is the `FALLBACK` (empty evidence), skip `RecommendationAgent.propose` and emit a `reasoning_step` explaining why no recommendations were produced, instead of running `propose` against a hollow diagnosis.
...
- **Files to touch:** `app/api/agent/route.ts` (L156–L161); `lib/agents/diagnostic.ts` (expose whether the result was `FALLBACK`).
...
- **What to build:** A small `lib/agents/pipeline.ts` that declares the investigation chain as a typed array of nodes `[{ name, run, consumes }]` and a runner that executes them in order, threading each node's output to the next — then have `route.ts` call the runner instead of the inline `if/await/await`.
...
- **Files to touch:** new `lib/agents/pipeline.ts`; `app/api/agent/route.ts` (replace L145–L162); `test/agents/pipeline.test.ts`.
...
blooming insights runs a deterministic chain of bounded agents. The route (`route.ts` L145–L162) fixes the order — diagnose then recommend — with plain `await` and a typed handoff. Each node (`runAgentLoop`, `base.ts` L48–L176) is an agent: the model chooses which tools to call and signals termination by emitting no `tool_use` block (L121), bounded by `maxTurns` and `maxToolCalls`. Inside an agent, the loop-then-`synthesize` pair (`diagnostic.ts` L73–L77) is a 2-step micro-chain. The architecture is the same idea at three scales: fixed spine, adaptive node, fixed fallback.
...
```
today (2 await):        becomes a graph when:
investigate             diagnose ──low?──→ reinvestigate ──┐
  ↓                          │high                         │
propose                      ↓                             ↓
...
- `app/api/agent/route.ts` L152–L159 — the chain: `await investigate` then `await propose(diagnosis)`.
...
- `lib/agents/diagnostic.ts` L73–L77 — the micro-chain: loop → synthesize → FALLBACK.
...
Scenario: a PM wants the system to run the diagnostic and recommendation steps *in parallel* to cut latency. Why does the current architecture forbid this, and what specifically would break? Check against `app/api/agent/route.ts` L156–L158 — note that `propose` takes `diagnosis` as an argument, so it *cannot* start until `investigate` resolves. Explain that the dependency is a data dependency, not a code-style choice, and that true parallelism would require a different decomposition.
