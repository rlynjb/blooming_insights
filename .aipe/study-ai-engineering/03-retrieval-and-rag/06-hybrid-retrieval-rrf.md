...
From memory, write the RRF formula, state the default `k`, and draw two short ranked lists fused into one. Explain why a document in both lists can beat a document ranked #1 in only one.

### Level 2 — Explain

Out loud: why does fusing by rank position avoid the cosine-vs-BM25 scale problem? What does the constant `k` control, and what does RRF discard that weighted-sum fusion keeps?

### Level 3 — Apply

Scenario: "find past investigations like this mobile-checkout problem." Open `lib/mcp/tools.ts` L15–L25 (the sparse EQL leg) and imagine the dense leg from `05-dense-vs-sparse.md`. Name what each leg catches (exact "mobile"/`checkout_started` vs. paraphrase "cart drop-off on phones"), and how RRF fuses them. Explain why the analytics agents stay sparse-only.

### Level 4 — Defend

A colleague wants to add hybrid search to the analytics agents "for completeness." Argue why this is pure overhead for exact aggregates (no meaning axis, a second index for zero recall gain) and confine hybrid to the free-text investigation corpus. Then defend RRF over weighted score-sum (no calibration needed).
