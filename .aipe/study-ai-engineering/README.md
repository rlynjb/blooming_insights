...
- **[04-agents-and-tool-use/](04-agents-and-tool-use/README.md)** (6 files) — agents vs chains, tool calling, ReAct, tool routing, agent memory, error recovery.
...
**Case A** (implemented — cited to real `file:line`): the agent loop and all four agents, tool calling, ReAct, tool routing, structured outputs, streaming, heuristic-before-LLM, the provider/transport seam, context budgeting, prompt chaining, LLM observability, exact-match caching, model-routing cost control, inter-call rate limiting, bounded retry.
