...
---
Updated: 2026-05-28 — refreshed the codebase-specific bullets only: `classifyIntent` dispatch `route.ts:136`→`:211`, query-flow block `:135–142`→`:210–216`, never-cached comment `:62`→`:126`, and noted the investigation path is now a separate `?step=diagnose`/`?step=recommend` two-step flow (not the chatbot surface). Generic architecture/scale/eval sections unchanged.
