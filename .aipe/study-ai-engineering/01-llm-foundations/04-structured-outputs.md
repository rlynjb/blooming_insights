...
`isAnomalyArray` (L17–L27) is stricter — it walks every element and checks nested fields (`change.value` is a number, `change.direction` is `'up'|'down'`, `severity` is in the `SEVERITIES` set). `isRecommendationArray` (L42–L53) validates the *id-less* shape, because the agent emits recommendations without an `id` and the code assigns it after validation (`lib/agents/recommendation.ts` L76):
...
**The synthesis instruction** (the in-loop nudge) is appended to the system prompt on the forced-final turn (`lib/agents/base.ts` L96–L98). For the diagnostic agent it reads (`lib/agents/diagnostic.ts` L62–L66): "You have NO more tool calls available... Respond with ONLY a single JSON object in a ```json fence matching the diagnosis shape." This tells the model exactly what to emit and prohibits further exploration — so the loop's final turn usually produces fence-wrapped JSON that clears stages 1 and 2 directly.

**The dedicated `synthesize()` call** is the clean-context retry when the nudge fails. `DiagnosticAgent.synthesize` (`lib/agents/diagnostic.ts` L82–L121) is a *separate* `anthropic.messages.create` (L92) — no tools, no loop history. It formats the gathered `toolCalls` as evidence text and asks for only the JSON:
...
Why a fresh call instead of one more loop turn: the loop's message history is full of `tool_use`/`tool_result` pairs and partial reasoning — the model has momentum toward "I should query more." A clean single-turn call with no tools and no history breaks that momentum; it sees only evidence + schema + "output JSON" and reliably complies. The recommendation agent has the identical structure (`lib/agents/recommendation.ts` L82–L127, call at L96).

The whole contract assembles in the fallback chain (`lib/agents/diagnostic.ts` L73–L77):
...
│       validate.ts L17–53   (id NOT validated; assigned after)        │
│           │ valid              │ null / threw                        │
│           ▼                    ▼                                     │
...
│                                                diagnostic.ts L15–19  │
│                                                                       │
│  chain: tryParse(finalText) ?? synthesize() ?? FALLBACK  (L73–77)    │
└────────────────────────────────────────────────────────────────────────┘
...
- **Validate (type guards):** `isAnomalyArray` L17–L27, `isDiagnosis` L29–L35, `isRecommendationArray` L42–L53 — all in `lib/mcp/validate.ts`. The recommendation guard validates the id-less shape; `SEVERITIES` (L15) and `FEATURES`/`CONFIDENCE` (L37–L38) back the enum checks.
...
- **Dedicated repair call (diagnostic):** `DiagnosticAgent.synthesize(anomaly, toolCalls)` — `lib/agents/diagnostic.ts` L82–L121; tool-less `create` at L92, `max_tokens: 2048` at L94.
- **Dedicated repair call (recommendation):** `RecommendationAgent.synthesize(anomaly, diagnosis, toolCalls)` — `lib/agents/recommendation.ts` L82–L127; `create` at L96; ids assigned post-validation via `crypto.randomUUID()` at L76, capped to 3.
- **The contract chains:** diagnostic `tryParseDiagnosis ?? synthesize ?? FALLBACK` — `lib/agents/diagnostic.ts` L73–L77; recommendation `tryParseRecommendations ?? synthesize` then `[]` — `lib/agents/recommendation.ts` L69–L76; monitoring `parseAgentJson` + `isAnomalyArray` else `[]` — `lib/agents/monitoring.ts` L85–L92.
...
**What the alternative would have cost.** Coupling the final-artifact contract to one provider's structured-output feature, with different shapes and limits per vendor, and a contract that is harder to exercise in the 125-test suite without the live API. Notably the codebase already uses native tool-use for *input* (data retrieval) — so the choice was specifically to keep the *output* side in portable application code.
...
- **Codebase uses:** `lib/mcp/validate.ts` L17–L53 — hand-written `v is T` predicates, one per shape.
- **Why it's here:** to convert `parseAgentJson`'s `unknown` into a typed value with a runtime shape proof, validating the id-less recommendation shape specifically.
...
- **Codebase uses:** `lib/agents/diagnostic.ts` L82–L121, `lib/agents/recommendation.ts` L82–L127 — a fresh, tool-less `create` that re-derives the artifact from evidence text.
...
`parseAgentJson` (`lib/mcp/validate.ts` L3–L13) tries the fence regex first, capturing the body inside ```json``` — that's the common case. Then `isDiagnosis` (L29–L35) proves the shape. If both succeed, `tryParseDiagnosis` returns a typed `Diagnosis`; otherwise `null`.
...
- `lib/mcp/validate.ts` L17–L53 — the three type guards; recommendation validates the id-less shape.
- `lib/agents/base.ts` L96–L98 — `synthesisInstruction` appended on the forced-final turn.
- `lib/agents/diagnostic.ts` L82–L121 — `synthesize()`: clean-context, tool-less repair.
- `lib/agents/diagnostic.ts` L73–L77 — the contract chain: `tryParse ?? synthesize ?? FALLBACK`.
...
Out loud: why does `isRecommendationArray` validate a shape *without* an `id` (`lib/mcp/validate.ts` L42), and where does the `id` come from (`lib/agents/recommendation.ts` L76)? What bug does this split prevent?
