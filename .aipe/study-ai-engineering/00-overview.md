...
Updated: 2026-06-19 — Olist removal (PR #8 / 62c24d7) collapsed the eval framing: opening paragraph reverts ("a 4-pillar eval suite under `eval/`" → "the eval suite is gone"), eval-pillar legend entry replaced with a RETIRED summary, DataSource adapter seam legend entry renamed Olist → `SyntheticDataSource` (`live-sql` → `live-synthetic`). AptKit integration is the new ground: opening paragraph now names `@aptkit/core@0.3.0` + the three adapter classes; the `runAgentLoop` legend entry collapsed into a new "AptKit primitive adapters" legend entry pointing at `04-agents-and-tool-use/09-aptkit-primitive-adapters.md`. Sub-section pointer for 04 now names the new AptKit-adapters file; 05/07 pointers flip to RETIRED on the eval entries.
