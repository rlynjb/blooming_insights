...
### Why the 125 Vitest tests are not evals
...
```
what the 125 tests assert          what they NEVER assert
─────────────────────────────      ──────────────────────────────────
...
This set is especially apt for blooming insights because of one concrete, present hole: the free-form `?q=` path is unsanitized. In `app/api/agent/route.ts` the query is only `.trim()`'d (L54) and passed straight through as the agent's user prompt (`QueryAgent.answer(q, ...)` at L139, into `lib/agents/query.ts`). There is no instruction-injection filter, no allowlist, no escaping. An adversarial set is how you *prove* the model's behaviour on inputs like:
...
──────────────────────────────      ──────────────────────────────────
test/ — 125 Vitest unit tests       test/ — unchanged (plumbing)
...
└───────────────────────────────────────────────────────────────────────┘
   (contrast: the 125 tests in test/ inject buildFakeMcp + fake Anthropic
...
| Dimension | Golden + adversarial + regression sets | Existing 125 Vitest tests | Manual demo review ("it looks good") |
|---|---|---|---|
...
**What the alternative would have cost.** Leaning on the unit suite as if it were an eval suite is worse than having nothing, because green tests create false confidence: an engineer sees 125 passing tests and believes quality is guarded when not one test looks at quality. Manual demo review is honest about its limits but does not scale and does not survive the reviewer leaving.
...
- **Codebase uses:** nothing — the `?q=` path (`app/api/agent/route.ts` L54, L139) is untested against injection.
...
- **Files to touch:** `evals/fixtures/golden.json`, `evals/fixtures/adversarial.json`, `evals/fixtures/regression.json`, `evals/README.md` (format spec); reference the real shapes in `lib/mcp/types.ts` (`Anomaly` L38–L44, `Diagnosis` L47–L52, `Recommendation` L58–L66) so cases match production types.
...
- **What to build:** drive the `?q=` path (`app/api/agent/route.ts` L135–L142) with an injection input, record the model's actual behaviour, and add it to `regression.json` with the corrected expected behaviour — making the first concrete regression guard the codebase's known security gap.
...
An eval set is a fixed collection of inputs paired with expected answers, run against the *real* model to score output quality — distinct from a unit test, which mocks the model and asserts plumbing. blooming insights has 125 Vitest tests that inject fakes and check control flow and output shape (`isDiagnosis` at `lib/mcp/validate.ts` L29–L35 checks structure, not truth); not one scores an answer, so the codebase has no eval set. Golden sets measure quality, adversarial sets measure robustness (especially apt for the unsanitized `?q=` path at `app/api/agent/route.ts` L54), and regression sets freeze every past failure. The three live in a new `evals/` directory and are the buildable target.
...
- 125 Vitest tests inject fakes and assert plumbing — not one scores an answer (Case B).
- `isDiagnosis` (`lib/mcp/validate.ts` L29–L35) checks shape, not truth.
- `?q=` is `.trim()`'d only (`app/api/agent/route.ts` L54) → adversarial set is apt.
...
Out loud: why does a green run of all 125 Vitest tests tell you nothing about whether `DiagnosticAgent` produces a *correct* diagnosis? Use `isDiagnosis` to make the point concrete.
...
A colleague says "we already have 125 tests, we don't need evals." Argue the distinction: name what the tests guard, name what they cannot, and give the concrete change (a model swap or a `?q=` injection) that the test suite would pass through silently while an eval set would catch.
