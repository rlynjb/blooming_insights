...
**Zoom out — the bigger picture.** The spacing gate is a metronome — it ticks once a second whether the orchestra is playing or sitting silent; backpressure is the conductor's hand going up when the brass section is overwhelming the strings. Both make the music "slow down," but only one of them is *reacting* to what the room is doing. Three different patterns live at the provider/transport layer and everyone confuses them: *rate limiting* (the server says "no more than N per period"), *throttling* (the client says "I'll send at most N per period to be a good citizen"), and *backpressure* (the consumer signals the producer to slow down when the queue fills). They look similar at first glance — all three involve some form of "wait before sending more" — but they solve different problems and require different machinery. blooming insights has the second one (client-side throttling / rate-limit compliance, the metronome) and does not have the third (backpressure, the conductor's hand). The first one is enforced by Bloomreach upstream, not by us. Naming them correctly is what lets you reason about what would break under what conditions.
