...
The second thing that breaks is mine, and I own it cleanly: **my state is per-instance, in-memory.** The 60s response cache lives inside one `McpClient` instance (`lib/mcp/client.ts`, the `cache` Map on line 80). Cached investigations live in a module-level `Map` in `lib/state/investigations.ts` (line 11). On Vercel that means a warm instance has the cache; a cold start re-bootstraps the schema from scratch; and two requests that land on different instances do not share anything. I already worked around the worst of this — an investigation's source insight is handed to the agent route through the URL (`?insight=`), parsed in `resolveAnomaly` (`lib/api/agent` route, lines 37-62), *because* I knew in-memory state would not survive Vercel's per-instance memory between the feed and the investigate page. That is a band-aid, not a store.
...
**Questions covered:**
- *What breaks first at 10x users?* — Bloomreach's per-user rate limit + token revocation (not my code); then my per-instance in-memory cache/investigation state. Add Redis when I go multi-instance.
...
**What you'd change:** Lift state (cache, investigations, OAuth tokens) out of per-instance memory into a shared store the moment I go multi-instance — and don't build anything to fight the upstream rate limit, because that number isn't mine.
