...
```
  ...
  └─────────────────────────┬────────────────────────────┘
                            │  HTTPS (cookies)
  ┌─ Service layer ─────────▼────────────────────────────┐
  │  Vercel instance(s) · in-memory Maps · McpClient      │
  └─────────────────────────┬────────────────────────────┘
                            │  HTTPS (3 different partners)
  ┌─ Provider layer ────────▼────────────────────────────┐
...
- **Seam C — instance ↔ Bloomreach MCP.** Ownership flips from "ours (cache, lastCallAt counter, spacing budget)" to "theirs (rate-limit window, workspace data, the truth)."
- **Seam D — instance ↔ Anthropic.** Ownership flips from "ours (the prompt + tool schemas we built)" to "theirs (the model's output, their rate limits)."
- **Seam E — instance ↔ Bloomreach IdP.** Ownership flips from "ours (the PKCE verifier, the redirect_uri we registered)" to "theirs (the auth code, the access + refresh tokens)."

Seams C, D, E are the three real distributed boundaries. Seam B is the gap. Seam A is the workaround for Seam B (the client carries state precisely because the server can't be trusted to remember it).
...
```
  ...
  └─────┬─────────┬─────────┬─────┘   └────── (no link between them) ───┘
        ...
        │ MCP     │ Anthr.  │ IdP
        ▼         ▼         ▼
  ┌─ Bloomreach ─┐  ┌─ Anthropic ─┐  ┌─ Bloomreach IdP ─┐
  │  MCP server   │  │  API         │  │  OAuth + DCR      │
  │  1 req/s/user │  │  rate-       │  │  + PKCE           │
  │  GLOBAL       │  │  limited     │  │  authorize URL +  │
  │  429 carries  │  │  + variable  │  │  code exchange    │
  │  retry hint   │  │  latency     │  │                   │
  └───────────────┘  └──────────────┘  └───────────────────┘
...
- `02-partial-failure-timeouts-and-retries.md` — Seam C in depth: how `McpClient` handles the 429
