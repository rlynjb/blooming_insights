...
```
  ...
  │  - synthetic data source (in-process, no persistence)          │
  └────────────────────────────────────────────────────────────────┘
...
### 10. LOW — Bloomreach is a single point of failure with no caching of last-known-good
...
**The fix shape, if we cared.** Persist last-successful briefing to a shared store; serve it as "stale, last refreshed at T" when live calls fail. Today the demo fallback (`?demo=cached`) covers the no-credentials case but not the upstream-down case mid-session. The synthetic data source covers neither — it's a separate dev/test path, not a live-failure backstop.
...
```
  classical database red flags that don't apply

  ✗ "N+1 in the ORM"                  — no ORM. N+1 exists in the agent
                                         loop (section 04 Move 2c) and is
                                         accepted by design.

  ✗ "missing indexes on hot queries"  — no queries; no SQL; no indexes
                                         to miss.

  ✗ "deadlocks under contention"       — no locks at all. Node's event loop
                                         is the closest scheduler.

  ✗ "WAL not synced; data loss risk"  — nothing durable to WAL in the first
                                         place. The cookie + git fixtures are
                                         the entire durability story.

  ✗ "stale replica reads"             — no replicas. cross-instance
                                         divergence (named in #2) is the
                                         analogous problem at a higher
                                         altitude.

  ✗ "backup not tested"               — no data to back up beyond the git-
                                         committed demo fixtures, which are
                                         "tested" every clone.

  ✗ "schema migration broke prod"     — no schema; no migrations.
```
...
- `01-database-systems-map` — the layout being audited
