...
```
  ...
  │  - schema singleton                                            │
  │  - in-process state Maps (insights / investigations / anomalies)│
...
```
  classical database red flags that simply don't apply here

  ✗ "N+1 in the ORM"                  — no ORM. N+1 exists in the agent
                                         loop (section 04 Move 2c) and is
                                         accepted by design.

  ✗ "missing indexes on hot queries"  — no queries. Maps give O(1) primary
                                         key lookup; no secondary access
                                         patterns exist.

  ✗ "deadlocks under contention"       — no locks. no engine. Node's event
                                         loop is the closest thing to a
                                         scheduler.

  ✗ "WAL not synced; data loss risk"  — nothing durable enough to write a
                                         WAL for. Cookies are durable but
                                         not via a log.

  ✗ "stale replica reads"             — no replicas. cross-instance
                                         divergence is named in #2 but it's
                                         a different family of problem.

  ✗ "backup not tested"               — no backups. no data to back up.
                                         demo fixtures are git-versioned.
