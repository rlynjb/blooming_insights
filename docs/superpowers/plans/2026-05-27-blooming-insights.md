...
- **Known limitation (same family as Phase 2):** the diagnosis *conclusion* often falls back to "insufficient data" because the agent exhausts its 6-call budget mid-investigation (and hits EQL syntax limits like unsupported `customers matching`) without emitting a final structured `Diagnosis`. The streaming trace works regardless. Follow-ups to land real conclusions: (a) the same `execution_time` stale-data anchoring; (b) a stronger forced-synthesis nudge so the budget-capped final turn reliably emits the `Diagnosis` JSON; (c) tighter EQL syntax guidance in the prompt (avoid `customers matching`); possibly (d) a slightly higher diagnostic budget. Also: each investigation runs live (~42s + Anthropic cost) — for a reliable demo, cache investigations like `demo-insights.json`.
