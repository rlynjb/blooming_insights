...
- **Investigation caching (RESOLVED for the demo, commit `77da708`/`d38a420`):** `lib/state/investigations.ts` caches each insight's full `AgentEvent[]`. `/api/agent` is cache-first: a hit **replays** the saved events with a ~180ms inter-event delay (~6s, animated) and needs **no auth and no Anthropic credits**; a miss runs live and saves on success (next click instant). All **6 demo cards** are pre-seeded in committed `demo-investigations.json` (each: diagnosis + 3 recommendations). Dev live-runs persist to a gitignored `.investigation-cache.json`. Verified: creds-free replay in 5.7s.

**Phase 5 status (2026-05-27):** Built and verified live. Global query box. `lib/agents/intent.ts` (`parseIntent` TDD + `classifyIntent` via Haiku). `lib/agents/query.ts` `QueryAgent.answer(query, intent, hooks)` — broad `queryTools` (monitoring∪diagnostic∪recommendation), returns the loop's final prose answer (no JSON shape). `/api/agent?q=...` branch: classifies intent → streams the query agent's reasoning → emits the answer as a `coordinator` `reasoning_step` of `kind:'conclusion'` → `done` (live; not cached). UI: `components/chat/QueryBox.tsx` (fixed-bottom) + `StreamingResponse.tsx` (pinned result on the feed, reuses `ReasoningTrace`, collapsible "show reasoning"). Verified live: "which countries had the most purchase revenue…" → 34s, intent=monitoring, 3 EQL calls, grounded answer (UK now #1 +12%, USA −31%) with the agent anchoring to the populated 90/180-day windows on its own. 125 unit tests pass; build clean.

**LATENCY (live investigation path):** a full live investigation (diagnostic + recommendation, back-to-back, each ~capped tool calls @ ~1.1s + dedicated synthesis) took **~115s** — well over Vercel's 60s `maxDuration`. It streams progress so it's watchable locally, but for a deployable/snappy demo the fix is to **cache investigations** (snapshot diagnosis + recommendations per insight id, like `demo-insights.json`; serve cached-first, regenerate in the background). This is the highest-value next infra task. Secondary levers: run diagnostic + recommendation concurrently is NOT possible (recommendation needs the diagnosis), but the recommendation agent rarely needs tool calls (it reasons from the diagnosis) — consider `maxToolCalls: 1–2` to trim its latency.
